...
/**
 * 1. Perform a numeric range query if start and end look like numbers. 2. Did
 * _not_ import the setLowercaseExpandedTerms(false) for KeywordAnalyzer from
 * ClouseauQueryParser as this is now fixed in Lucene, the normalize() method of
 * the analyzer is where lower-casing (or not) is applied and the
 * KeyWordAnalyzer does not perform case-folding for expanded terms.
 * 
 */
public final class Search3QueryParser extends QueryParser {
    ...
    static {
        ...
        final String regexp = ("[\\x00-\\x20]*" + "[+-]?(" + "NaN|"
              + "Infinity|" + "((("
              + digits
              + "(\\.)?("
              + digits
