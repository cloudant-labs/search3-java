...
public class SearchServer extends AbstractServer {
    ...
    private class SearchServerHandler extends SimpleChannelInboundHandler<HttpObject> {
        ...
        protected void channelRead0(final ChannelHandlerContext ctx, final HttpObject msg) throws Exception {
            if (msg instanceof FullHttpRequest) {
                ...
                case "/Search/GroupSearch": {
                    final GroupSearchRequest request = decode(GroupSearchRequest.getDefaultInstance(), req);
                    messageType = "group_search_response";
                    result = encode(search.groupSearch(request));
                    break;
                }
                case "/Search/UpdateDocument": {
                    final DocumentUpdateRequest request = decode(DocumentUpdateRequest.getDefaultInstance(), req);
                    messageType = "session_response";
                    result = encode(search.updateDocument(request));
                    break;
                }
                case "/Search/DeleteDocument": {
                    final DocumentDeleteRequest request = decode(DocumentDeleteRequest.getDefaultInstance(), req);
                    messageType = "session_response";
                    result = encode(search.deleteDocument(request));
                    break;
                }
                case "/Search/SetUpdateSequence": {
                    final SetUpdateSeqRequest request = decode(SetUpdateSeqRequest.getDefaultInstance(), req);
                    messageType = "session_response";
        ...
        private <T extends MessageLite> T decode(final T prototype, final FullHttpRequest req) throws InvalidProtocolBufferException {
        ...
        private FullHttpResponse error(final Type type, final String reason) {
            final ErrorResponse error = ErrorResponse.newBuilder().setType(type).setReason(reason).build();
            final ByteBuf body = wrappedBuffer(error.toByteArray());
            final FullHttpResponse result = new DefaultFullHttpResponse(
