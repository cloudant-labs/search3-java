...
public class SearchTest extends BaseFDBTest {
  ...
  public static Collection<SearchHandlerFactory> factories() {
    return Arrays.asList(
        new SearchHandlerFactory[] {
          /* new FDBIndexWriterSearchHandlerFactory(), */
  ...
  public void basicSearch() throws Exception {
    try (final Search search = new Search(config)) {
      ...
      final SearchRequest searchRequest =
          SearchRequest.newBuilder().setIndex(index).setQuery("foo:bar").setLimit(25).build();
  ...
  public void sortByDistanceSearch() throws Exception {
    try (final Search search = new Search(config)) {
      ...
      search.updateDocument(
          update(index, "foo", field("lon", 0.5, true, false), field("lat", 57.15, true, false)));
      search.updateDocument(
          update(index, "bar", field("lon", 10.0, true, false), field("lat", 57.15, true, false)));
      search.updateDocument(
          update(index, "zzz", field("lon", 3.0, true, false), field("lat", 57.15, true, false)));
    ...
    try (final Search search = new Search(config)) {
      ...
      final Index index = Index.newBuilder().setPrefix(ByteString.copyFrom(prefix)).build();
      {
        ...
        final GroupSearchRequest groupSearchRequest =
            GroupSearchRequest.newBuilder()
                .setIndex(index)
                .setGroupBy("category")
                .setQuery("category:foobar")
                .setLimit(25)
                .setGroupLimit(25)
                .build();
  ...
  private DocumentUpdateRequest update(
      final Index index, final String id, final DocumentField... fields) {
  ...
  private DocumentField field(
      final String name, final Object value, final boolean analyzed, final boolean facet) {
    return DocumentField.newBuilder()
        .setName(name)
